...
class Activity_englishsongs : AppCompatActivity() {
    private lateinit var database: DatabaseReference
    private lateinit var songAdapter: adapterEglishsongs
    private lateinit var songList: MutableList<String>
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_englishsongs)
        // Initialize Firebase Database
        database = FirebaseDatabase.getInstance().reference

        // Initialize RecyclerView
        val recyclerView = findViewById<RecyclerView>(R.id.rvEnglishSongList)
        recyclerView.layoutManager = LinearLayoutManager(this)
        songList = mutableListOf()
        songAdapter = adapterEglishsongs(songList)
    ...
    private fun fetchBollywoodSongs() {
        database.child("categories").child("english").addValueEventListener(object :
            ValueEventListener {
